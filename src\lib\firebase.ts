...
export type PersistenceMode = "local-first" | "production";

const PERSISTENCE_MODE_STORAGE_KEY = "alco_persistence_mode";
const DEFAULT_PERSISTENCE_MODE: PersistenceMode = "local-first";
...
export function getPersistenceMode(): PersistenceMode {
  try {
    const saved = localStorage.getItem(PERSISTENCE_MODE_STORAGE_KEY);
    if (saved === "production" || saved === "local-first") {
      return saved;
    }
  } catch (error) {
    console.warn("Failed to read persistence mode:", error);
  }
...
export function setPersistenceMode(mode: PersistenceMode) {
  try {
    localStorage.setItem(PERSISTENCE_MODE_STORAGE_KEY, mode);
    window.dispatchEvent(new CustomEvent("alco_persistence_mode_changed", { detail: { mode } }));
  } catch (error) {
    console.warn("Failed to save persistence mode:", error);
  }
}

export function getPersistenceRuntimeMeta() {
  const mode = getPersistenceMode();
  return {
    mode,
    label: mode === "production" ? "Production Intended" : "Local First",
    description:
      mode === "production"
        ? "Mode ditandai untuk produksi, namun driver aktif saat ini masih fallback lokal."
        : "Mode kerja utama saat ini adalah penyimpanan lokal browser.",
    backendReady: false,
    activeDriver: "mock-firestore"
  };
}
...
function stampPersistenceMeta(data: any) {
  const runtime = getPersistenceRuntimeMeta();
  return {
    ...data,
    persistenceMeta: {
      mode: runtime.mode,
...
export function setDoc(docRef: DocumentReference, data: any, options?: { merge?: boolean }) {
  ...
  let newData = stampPersistenceMeta({ ...data });
  if (options?.merge && dbState[path]) {
    newData = stampPersistenceMeta(deepMerge(dbState[path], data));
  }
...
export function updateDoc(docRef: DocumentReference, data: any) {
  ...
  const newData = stampPersistenceMeta(deepMerge(existing, data));
  dbState[path] = newData;
...
export function addDoc(collectionRef: CollectionReference, data: any) {
  ...
  const dbState = getLocalDB();
  dbState[docPath] = stampPersistenceMeta(data);
  saveLocalDB(dbState);
  triggerSnapshot(docPath, dbState[docPath]);
