...
export default function AdsContentStep({ project, onSaveProject }: AdsContentStepProps) {
  ...
  const saveStateToProject = (extraUpdates: any = {}) => {
    ...
    const dataToSave = {
      ...baseDataToSave,
      ...extraUpdates,
      metaAdsCampaignPack: buildMetaAdsCampaignPack(merged)
    };
  ...
  const handleExportCampaignPack = () => {
    try {
      const { fileName, blob } = downloadMetaAdsCampaignPack({
        ...project,
        adsInputState: {
      ...
      toast.success("Meta Ads Campaign Pack berhasil diunduh.");
    } catch (error) {
      console.error(error);
      toast.error("Gagal mengekspor Meta Ads Campaign Pack.");
    }
  };
      ...
      <div className="relative p-8 md:p-12 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden self-center mb-8">
        ...
        <div className="relative z-10 space-y-4 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            ...
            <Button
              onClick={handleExportCampaignPack}
              variant="outline"
              className="border-emerald-700 hover:border-emerald-500 hover:bg-emerald-800/30 text-emerald-200 font-extrabold uppercase tracking-wider text-[11px] h-12 px-6 rounded-xl gap-2"
            >
