...
export const AGENT_PROMPTS = {
  ...
  OPTIMIZE_ADS: "Act as a Conversion Rate Optimization (CRO) expert. Refine the provided ad content to make it significantly stronger, particularly optimizing the hooks and headlines for maximum curiosity and conversion power. Maintain the Indonesian language. You must return a flat JSON object with the exact same structure and keys as the input (such as type, headlines, primary_text, cta_direction, and particularly the hook fields: viral_hook, scroll_stopper_hook, curiosity_hook, emotional_hook, urgency_hook) containing the newly optimized content. Do not change the JSON schema, do not translate keys, and do not nest fields inside any outer key.",
